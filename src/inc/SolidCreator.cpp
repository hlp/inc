...
namespace inc {
    ...
    void SolidCreator::update() {
        if (cb_[0] != last_cb_[0]) {
            ...
            // create matrix
            create_sphere_matrix(ci::Vec3f(0.0f, 150.0f, 0.0f), ci::Vec3f::one() * 3.0f,
                4, 4, 4);
            last_cb_[2] = cb_[2];
        }
    }
    ...
    void SolidCreator::create_sphere_matrix(ci::Vec3f pos, ci::Vec3f size,
        int w, int h, int d) {
        std::tr1::shared_ptr<std::deque<SolidPtr> > d_ptr = 
            SolidFactory::create_soft_sphere_matrix(pos, size, w, h, d);
